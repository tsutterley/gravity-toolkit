...
def kernel_degree_one(
    ...
    for i in range(n_harm):
        ...
        for j in range(n_harm):
            ...
            output['covariance'][i, j, :] = (
                rho_e * rad_e * DMAT / (4.0 * np.pi * dfactor[1])
            )
...
def ncdf_covariance(output, **kwargs):
    ...
    nc['geocenter'] = fileID.createVariable('geocenter', 'i', ('geocenter',))
    # spherical harmonics
    nc['covariance'] = fileID.createVariable(
        'covariance', 'd', ('lm', 'lm', 'geocenter')
    )
    ...
    fileID.summary = (
        'Geocenter covariance coefficients.  Geocenter '
