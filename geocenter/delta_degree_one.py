...
def delta_degree_one(
            ...
            for l in range(m, LMAX + 1):  # LMAX+1 to include LMAX
                ...
                for cs, csharm in enumerate(['clm', 'slm']):
                    ...
                    smth = gravtk.time_series.smooth(
                        GSM_Ylms.time, val1[l, m, :], HFWTH=HFWTH
                    )
    ...
    l2 = slice(2, LMAX + 1)
    pconv = np.einsum(
        'lmh...,lm...->mh...', plmout[l2, :, :], delta_Ylms.ilm[l2, :]
    )
    ...
    while (eps > eps_max) and (n_iter < max_iter):
        ...
        l1 = slice(1, LMAX + 1)
        pconv = np.einsum(
            'lmh...,lm...->mh...', plmout[l1, :, :], delta_Ylms.ilm[l1, :]
        )
        ...
        if FINGERPRINT:
            ...
            eustatic = (
                gravtk.geocenter().from_harmonics(slf_Ylms).scale(dfactor[1])
            )
        else:
...
def print_global(fid, PROC, DREL, MODEL, SLR, S21, month):
    ...
    summary.append(
        (
            'Geocenter error coefficients derived from {0} mission '
