...
def combine_HEX_TWC_errors(
    ...
    for m in MODEL:
        ...
        s = 'GLDAS_{0}{1}{2}_TWC_SPH_CAP_MSCNS_L{3:d}_{4:03d}-{5:03d}'.format(
            *a
        )
    ...
    for i in region:
        ...
        for k in cap[i]:
            for m in MODEL:
                ...
                TWC_mass[m][i][ind] += (
                    TWC[m][k][:, 2] - TWC[m][k][m0314, 2].mean()
                )
        ...
        for t in range(n_mon):
            ...
            print(
                f'{mon[t]:03d} {tdec[t]:12.4f} {thick_error:14.6f}', file=fid2
            )
...
def arguments():
    ...
    # GLDAS model version
    parser.add_argument(
        '--version', type=str, default='2.1', help='GLDAS model version'
    )
