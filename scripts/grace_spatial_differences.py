...
def grace_spatial_differences(
    ...
    for i, FLAG in enumerate(FLAGS):
        ...
        error = combined.copy()
        error.data = np.copy(combined.error)
        e1[i] = error.sum(power=2.0).scale(1.0 / N).power(0.5)
        # significance means
        AICx0[i] = (
            gravtk.spatial().from_list(dinput['AIC_x0'], date=False).mean()
        )
        AICx1[i] = (
            gravtk.spatial().from_list(dinput['AIC_x1'], date=False).mean()
        )
        AICx2[i] = (
            gravtk.spatial().from_list(dinput['AIC_x2'], date=False).mean()
        )

    # set invalid values for masked variables
    ii, jj = np.nonzero(
        (np.abs(x1[0].data) <= e1[0].data)
