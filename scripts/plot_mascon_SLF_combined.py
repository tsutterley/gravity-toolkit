...
def plot_mascon_SLF_combined(base_dir, PROC, DREL, START_MON, END_MON, MISSING):
    ...
    ysea = dict(
        GIS=[14, -2, -2], WAIS=[8, -1, -1], EAIS=[1, -4, -1], APIS=[1, 0, -1]
    )
    ...
    for reg, ax in ax1.items():
        ...
        for i, F in enumerate(FLAG):
            ...
            mm_sealevel_error = (
                1e18
                * np.sqrt(
                    grace_error**2 + (tstar * gia_corrected_error[-1]) ** 2
                )
                / ocean_area
            )
        ...
        seaticks = np.arange(
            ysea[reg][0], ysea[reg][1] + ysea[reg][2], ysea[reg][2]
        )
        ax2[reg].set_yticks(seaticks)
        ax2[reg].set_ylim(
            -1e18 * ylimits[reg][0] / ocean_area,
            -1e18 * ylimits[reg][1] / ocean_area,
        )
        ax2[reg].tick_params(
        ...
        ax2[reg].set_ylabel(
            'Equivalent Sea Level [mm]', labelpad=seapad[reg], color='black'
        )
    ...
    fig.subplots_adjust(
        left=0.08, right=0.99, bottom=0.04, top=0.99, hspace=0.08, wspace=0.28
    )
