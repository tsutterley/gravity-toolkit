...
def plot_mascon_SLF_iterations(
    base_dir, PROC, DREL, START_MON, END_MON, MISSING, SCENARIOS=False
):
    # directory setup
    ...
    gia_files['AW13-ICE6G'].append(
        ['GIA', 'AW13', 'ICE6G', 'stokes.ice6g_.1_10.']
    )
    gia_files['AW13-ICE6G'].append(
        ['GIA', 'AW13', 'ICE6G', 'stokes.ice6g_1._10.']
    )
    gia_files['AW13-ICE6G'].append(
        ['GIA', 'AW13', 'ICE6G', 'stokes.ice6g_.1_1.']
    )
    gia_files['AW13-ICE6G'].append(
        ['GIA', 'AW13', 'ICE6G', 'stokes.ice6g_1._1.']
    )
    gia_files['AW13-ICE6G'].append(
        ['GIA', 'AW13', 'ICE6G', 'stokes.ice6g_GA.txt']
    )
    gia_files['ascii'] = []
    gia_files['ascii'].append(
        ['GIA', 'AW13', 'IJ05-R2', 'IJ05_R2_115_.2_1.5_ICE6G.txt']
    )
    gia_files['ascii'].append(
        ['GIA', 'AW13', 'IJ05-R2', 'IJ05_R2_65_.2_1.5_ICE6G.txt']
    )
    gia_files['Caron'] = []
    gia_files['Caron'].append(['GIA', 'Caron', 'expStokes_GIA.txt'])
    gia_files['ICE6G-D'] = []
    gia_files['ICE6G-D'].append(
        ['GIA', 'ICE6G', 'VersionD', 'Stokes_trend_High_Res.txt']
    )
    gia_files['ICE6G-D'].append(
        ['GIA', 'ICE6G', 'VersionD', 'Stokes_trend_VM5a_O512.txt']
    )
    gia_files['ICE6G'] = []
    gia_files['ICE6G'].append(
        ['GIA', 'ICE6G', 'VM5', 'Stokes_G_Rot_60_I6_A_VM5a']
    )
    gia_files['ICE6G'].append(
        ['GIA', 'ICE6G', 'VM5', 'Stokes_G_Rot_60_I6_A_VM5b']
    )
    gia_files['IJ05-R2'] = []
    ...
    landsea = gravtk.spatial().from_netCDF4(
        LSMASK, date=False, varname='LSMASK'
    )
    ...
    months = sorted(
        set(np.arange(START_MON, END_MON + 1)) - set(MISSING) - set(GAP)
    )
    nmon = len(months)
    ...
    # input leakage file
    leakage_file = mascon_dir.joinpath(
        'HEX_TMB_LEAKAGE_SPH_CAP_MSCNS_L60',
    ...
    tablefile = filepath.joinpath(
        'table_1_{0}_{1}_obp_atm_twc.txt'.format(PROC, DREL)
    )
    ...
    for h, reg, rem in zip(HEM, regions, remove):
        ...
        input_file = ff.format(
            'Caron_Error', reg, '', atm_str, ocean_str, LMAX, gw_str, ds_str
        )
        ...
        for j, g in enumerate(GIA[h]):
            ...
            for i, F in enumerate(FLAG):
                ...
                for k, GIA_FILE in enumerate(gia_files[g]):
                    gia_Ylms = gravtk.read_GIA_model(
                        base_dir.joinpath(*GIA_FILE), GIA=g
                    )
                ...
                for k, GIA_FILE in enumerate(gia_files[g]):
                    gia_corrected_variance += np.abs(
                        mass[:, k] - gia_corrected_mean
                    )
                # calculate GIA uncertainty as "worst-case" min max error
                for t in range(nmon):
                    gia_corrected_minmax[t] = np.abs(
                        np.max(mass[t, :]) - np.min(mass[t, :])
                    )
                ...
                if g == 'Caron':
                    ...
                    gia_corrected_error = gia_corrected_variance / (
                        np.float64(nRheology) - 1.0
                    )
                    gia_corrected_error = gia_corrected_error * np.sign(
                        tdec - tdec.mean()
                    )
                    gia_corrected_minmax = (
                        gia_corrected_minmax * np.sign(tdec - tdec.mean()) / 2.0
                    )
                    gia_error_rate = (
                        gia_corrected_error[-1] - gia_corrected_error[0]
                    ) / (tdec[-1] - tdec[0])
                    gia_minmax_rate = (
                        gia_corrected_minmax[-1] - gia_corrected_minmax[0]
                    ) / (tdec[-1] - tdec[0])
                    # calculate GIA errors at confidence interval
                    # t.ppf parallels tinv in matlab
                    tstar = (
                        scipy.stats.t.ppf(1.0 - (alpha / 2.0), nRheology - 1.0)
                        if g in ('IJ05-R2', 'SM09')
                        else 2.0
                    )
                ...
                bfit = gravtk.time_series.regress(
                    tdec,
                    gia_corrected_mean,
                    ORDER=2,
                    CYCLES=[0.5, 1.0],
                ...
                ofit = gravtk.time_series.regress(
                    tdec,
                    gia_corrected_mean,
                    DATA_ERR=OBP_RMS,
                    ORDER=2,
                ...
                afit = gravtk.time_series.regress(
                    tdec,
                    gia_corrected_mean,
                    DATA_ERR=ATM_RMS,
                    ORDER=2,
                ...
                tfit = gravtk.time_series.regress(
                    tdec,
                    gia_corrected_mean,
                    DATA_ERR=TWC_RMS,
                    ORDER=2,
                ...
                ex2[i] = (
                    2.0 * bfit['error'][2]
                    + np.abs(leakage_fraction * cx2)
                    + np.abs(2.0 * ofit['error'][2])
                    + np.abs(2.0 * afit['error'][2])
                    + np.abs(2.0 * tfit['error'][2])
                )
                args = (iter_label[i], bx1[i], ex1[i], bx2[i], ex2[i])
                print(
                    '{0}\tx1={1:f}+/-{2:f}\tx2={3:f}+/-{4:f}'.format(*args),
                    file=fid,
                )
                ...
                print(
                    '\tmx1={0:f}+/-{1:f}\tmx2={2:f}+/-{3:f}'.format(*a),
                    file=fid,
                )
                # converting gigatonnes to milligrams then to mm sea level
                mm_sealevel = (
                    -1e18
                    * (gia_corrected_mean - gia_corrected_mean[0])
                    / ocean_area
                )
                cumulative_gia = tstar * (
                    gia_corrected_error[-1] - gia_corrected_error[0]
                )
                mm_sealevel_error = (
                    1e18
                ...
                print(
                    '\ttotal sea level: {0:f}+/-{1:f} mm'.format(*args),
                    file=fid,
                )
            # if running with all scenarios
            if SCENARIOS:
    ...
    for h, reg in REGION.items():
        ...
        SLF = (
            ['', '_SLF3', '_SLF6'] if (reg == 'GIS') else ['', '_SLF3', '_SLF5']
        )
        # leakage fraction for regions
        leakage_fraction = regional_leakage[reg]
        # read ocean bottom pressure leakage file
        ...
        for i, F in enumerate(FLAG):
            ...
            for k, GIA_FILE in enumerate(gia_files[g]):
                ...
                gia_str = gia_Ylms['title']
                input_file = ff.format(
                    gia_str, reg, '', atm_str, ocean_str, LMAX, gw_str, ds_str
                )
                dinput = np.loadtxt(mascon_dir.joinpath(subdir, input_file))
                mon[:] = dinput[:nmon, 0].astype(np.int64)
                tdec[:] = dinput[:nmon, 1]
            ...
            gia_corrected_minmax = gia_corrected_minmax * np.sign(
                tdec - tdec.mean()
            )
            gia_error_rate = (
                gia_corrected_error[-1] - gia_corrected_error[0]
            ...
            # plot all dates
            ax[h].plot(
                tnan, mnan, color=plot_colors[i], label=plot_label, zorder=2
            )
            ...
            mm_sealevel_error = (
                1e18 * np.sqrt(grace_error**2 + cumulative_gia**2) / ocean_area
            )
        ...
        vs = ax[h].axvspan(
            tdec[jj], tdec[kk], color='0.5', ls='dashed', alpha=0.15
        )
        vs._dashes = (6, 3)
        # add labels
        textprops = dict(size=14, weight='bold')
        at = matplotlib.offsetbox.AnchoredText(
            fig_text[h], prop=textprops, pad=0, frameon=False, loc=2
        )
        ax[h].add_artist(at)
        data_ticks = np.arange(np.floor(tdec[0]), np.ceil(tdec[-1]) + 1, 2)
        ax[h].xaxis.set_ticks(data_ticks)
        data_ticks = np.arange(
            ylimits[1] - 200, ylimits[0] - ylimits[2], -ylimits[2]
        )
        ...
        ax1[h].tick_params(
            axis='y', colors='black', which='both', direction='in'
        )

    # formatted ticks on N axis
    ax1['N'].yaxis.get_major_formatter().set_useOffset(False)
    ax1['N'].set_ylabel(
        'Equivalent Sea Level Contribution [mm]', labelpad=10, color='black'
    )
    ...
    fig.subplots_adjust(
        left=0.1, right=0.93, bottom=0.10, top=0.98, wspace=0.06
    )
    ...
    # create figure axis
    ax = {}
    fig, (ax['S'], ax['N']) = plt.subplots(
        num=1, ncols=2, sharey=True, figsize=(8, 3.5)
    )
    # y limits for plot
    ylimits = [-90, 190, 20]
    for h, reg in REGION.items():
        ...
        ax[h].xaxis.set_ticks(data_ticks)
        data_ticks = np.arange(
            ylimits[1] - 10, ylimits[0] - ylimits[2], -ylimits[2]
        )
        ax[h].yaxis.set_ticks(data_ticks[::-1])
        ax[h].set_xticks(np.arange(np.floor(tdec[0]), np.ceil(tdec[-1]), 2))
        # axlim = ax[h].set_xlim([np.floor(tdec[0]), np.ceil(tdec[-1])])
    ...
    # adjust plot to figure dimensions
    fig.subplots_adjust(
        left=0.08, right=0.98, bottom=0.11, top=0.98, wspace=0.06
    )
...
def monte_carlo_regress(
    ...
    for i in range(0, RUNS):
        ...
        satellite_error_rand = (
            1.0 - 2.0 * np.random.rand(nmax)
        ) * satellite_error
        ...
        MSE = (
            np.dot(
                np.transpose(data_simul - np.dot(DMAT, beta_mat[i, :])),
                (data_simul - np.dot(DMAT, beta_mat[i, :])),
            )
            / nu
        )
    ...
    for n in range(n_terms):
        beta_var[n] = (
            np.dot(
                np.transpose(beta_mat[:, n] - beta_mean[n]),
                beta_mat[:, n] - beta_mean[n],
            )
            / RUNS
        )
        # probability distribution of the regression
        p_beta = np.exp(-0.5 * (beta_mat[:, n] - beta_mean[n]) ** 2) / np.sqrt(
            2.0 * np.pi
        )
    ...
    if RMS:
        ...
        mean_err = np.sqrt(
            np.sum(beta_err**2.0, axis=0) / RUNS + tstar_mean * beta_var
        )
    else:
        # mean_std is the standard error
