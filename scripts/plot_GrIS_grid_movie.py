...
UPDATE HISTORY:
    ...
    Updated 09/2018: added glacier outlines from the Randolph Glacier Inventory
        use a custom mask_oceans function instead of default maskoceans
        determine bitrate automatically from the underlying utility
    ...
    Updated 03/2016: fix add_artist for date string.
        for GLACIERS parameter: plot GrIS outline only (not all GIMP glaciers)
    Updated 12/2015: MODIS mosaic of Greenland
    Updated 11/2015: different date label colors if plotting with background
        mosaic. switched from anchored text to coordinate-based text (no jiggle)
