...
def regional_spherical_caps(
    ...
    for i, cap_number in enumerate(num):
        ...
        if DATAFORM == 'ascii':
            ...
            # netcdf (.nc)
            Ylms.to_netCDF4(
                OUTPUT_FILE, date=False, verbose=VERBOSE, **attributes
            )
        elif DATAFORM == 'HDF5':
            # HDF5 (.H5)
            Ylms.to_HDF5(OUTPUT_FILE, date=False, verbose=VERBOSE, **attributes)
...
def output_error_log_file(input_arguments):
    ...
    args = (time.strftime('%Y-%m-%d', time.localtime()), os.getpid())
    LOGFILE = 'regional_spherical_cap_failed_run_{0}_PID-{1:d}.log'.format(
        *args
    )
    # create a unique log and open the log file
    DIRECTORY = pathlib.Path(input_arguments.output_directory)
...
def arguments():
    ...
    parser.add_argument(
        '--cap-radius', type=float, help='Spherical cap radius (degrees)'
    )
...
def main():
    ...
    try:
        ...
        output_files = regional_spherical_caps(
            args.lmax,
