...
def plot_mascon_SLF_timeseries(
    base_dir, PROC, DREL, START_MON, END_MON, MISSING, SMB=False
):
    # directory setup
    ...
    (
        fig,
        ((ax1['GIS'], ax1['NN'], ax1['NE']), (ax1['NW'], ax1['SW'], ax1['SE'])),
    ) = plt.subplots(
        num=2, nrows=2, ncols=3, sharex=False, sharey=False, figsize=(12, 7.5)
    )
    ylimits = {
        'NW': [-1200, 1100, 200],
        'NN': [-450, 450, 100],
    ...
    for reg, ax in ax1.items():
        ...
        TWC_file = ff.format(
            'GLDAS_TWC_RMS', reg, '', 'RAD1.5_', ocean_str, LMAX, gw_str, ds_str
        )
        ...
        for i, F in enumerate(VERSION):
            ...
            for d, m in enumerate(month):
                ...
                if valid:
                    (mm,) = np.nonzero(mon == m)
                    tnan[d] = tdec[mm]
                    mnan[d] = gia_corrected_mean[mm] - np.mean(
                        gia_corrected_mean
                    )
            # plot all dates
            ax.plot(
                tnan, mnan, color=plot_colors[i], label=plot_label, zorder=3 + i
            )
        ...
        if 'RACMO' in SMB:
            ...
            RACMO_file = ff.format(
                'RACMO2.3p2',
                'FGRN055_DS1km_v4.0_SMB_cumul_',
                reg,
                'RAD1.5_',
                ocean_str,
                LMAX,
            ...
            RACMO = np.loadtxt(
                base_dir.joinpath('RACMO', 'SMB1km_v4.0', subdir, RACMO_file)
            )[:nmon, :]
            # create a time series with nans for missing months
            tnan = np.full_like(month, np.nan, dtype=np.float64)
            mnan = np.full_like(month, np.nan, dtype=np.float64)
            ...
            # plot all dates
            ax.plot(
                tnan, mnan, color=plot_colors[2], label='RACMO2.3p2', zorder=5
            )

        # plot GSFC-fdm SMB time series for comparison
        if 'GSFC-fdm' in SMB:
            # read MERRA-2 hybrid surface mass balance file
            subdir = 'HEX_GSFC_FDM_{0}_{1}_{2}_L{3:d}'.format(
                'v1_2_1', 'gris', 'SMB_a', LMAX
            )
            MERRA2_file = ff.format(
                'GSFC_FDM',
                'v1_2_1_SMB_a_',
                reg,
                'RAD1.5_',
            ...
            MERRA2 = np.loadtxt(
                base_dir.joinpath(
                    'MERRA2_hybrid', 'v1.2.1', subdir, MERRA2_file
                )
            )
            # plot all dates
            ax.plot(
                MERRA2[:, 1],
                MERRA2[:, 2] - np.mean(MERRA2[:, 2]),
                color=plot_colors[3],
                label='GSFC-fdm v1.2.1',
                zorder=5,
            )
        ...
        if 'GEMB' in SMB:
            ...
            GEMB = np.loadtxt(
                base_dir.joinpath('GEMB', 'v1.2', subdir, GEMB_file)
            )
            # create a time series with nans for missing months
            tnan = np.full_like(month, np.nan, dtype=np.float64)
            mnan = np.full_like(month, np.nan, dtype=np.float64)
            ...
            # plot all dates
            ax.plot(
                tnan, mnan, color=plot_colors[4], label='GEMB v1.2', zorder=5
            )
        ...
        at = matplotlib.offsetbox.AnchoredText(
            fig_text[reg], prop=textprops, pad=0, frameon=False, loc=2
        )
        ax.add_artist(at)
        textprops = dict(size=14, weight='bold')
        at = matplotlib.offsetbox.AnchoredText(
            reg_text[reg], prop=textprops, pad=0, frameon=False, loc=1
        )
        ax.add_artist(at)
        # set ticks
    ...
    fig.subplots_adjust(
        left=0.0625, right=0.99, bottom=0.05, top=0.99, wspace=0.2, hspace=0.125
    )
    ...
    (
        fig,
        (
            (ax1['AIS'], ax1['APIS'], ax1['GH3']),
            (ax1['QML'], ax1['CpDc'], ax1['DDpi']),
        ),
    ) = plt.subplots(
        num=2, nrows=2, ncols=3, sharex=False, sharey=False, figsize=(12, 7.5)
    )
    ...
    for reg, ax in ax1.items():
        ...
        if 'RACMO' in SMB:
            ...
            RACMO = np.loadtxt(
                base_dir.joinpath(
                    'RACMO', 'XANT27_1979-2022', subdir, RACMO_file
                )
            )[:nmon, :]
            # create a time series with nans for missing months
            tnan = np.full_like(month, np.nan, dtype=np.float64)
    ...
    ax1 = {}
    ax2 = {}
    fig, (ax1['EAIS'], ax1['INTERIOR']) = plt.subplots(
        num=1, ncols=2, figsize=(8, 3.75)
    )
    ...
    # adjust plot to figure dimensions
    fig.subplots_adjust(
        left=0.08, right=0.99, bottom=0.1, top=0.99, wspace=0.2, hspace=0.125
    )
