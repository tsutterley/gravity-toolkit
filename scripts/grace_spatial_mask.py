...
def grace_spatial_mask(
    ...
    # masked trend values
    ii, jj = np.nonzero(
        (np.abs(dinput['x1'].data) <= dinput['x1'].error)
    ...
    ii, jj = np.nonzero(
        (np.abs(dinput['x2'].data) <= dinput['x2'].error)
    ...
    output['F_x1'].data = (
        (dinput['SSE_x0'].data - dinput['SSE_x1'].data)
        / (DOF['x0'] - DOF['x1'])
        * (dinput['SSE_x1'].data / DOF['x1'])
    )
    output['F_x2'] = dinput['SSE_x2'].zeros_like()
    output['F_x2'].data = (
        (dinput['SSE_x1'].data - dinput['SSE_x2'].data)
        / (DOF['x1'] - DOF['x2'])
        * (dinput['SSE_x2'].data / DOF['x2'])
    )
