...
def create_public_data(
    ...
    kwargs['attributes']['ROOT']['polar_motion_feedback'] = (
        'Kendall et al. (2005)'
    )
    ...
    SUMMARY = []
    SUMMARY.append(
        ('Sea level variations derived from {0} mission measurements').format(
            MISSION
        )
    )
    SUMMARY.append(
    ...
    # output to file
    FILE = 'SLF{0}{1}{2}_L{3:d}_{4:03d}-{5:03d}.{6}'.format(
        dset_str,
        gia_str,
        ocean_str,
        EXPANSION,
