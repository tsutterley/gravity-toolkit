#!/usr/bin/env python
"""
combine_HEX_mascon_timeseries.py (04/2024)
Calculates the time-series of mass change for spherical cap mascons
    from synthetic data
...
def combine_HEX_mascon_timeseries(
    LMAX,
    RAD,
    MMAX=None,
    DESTRIPE=False,
    ...
    for i in region:
        ...
        for t in range(n_mon):
            ...
            mbar_anomaly = total_mbar[t] - total_mbar.mean()
            print(
                f'{mon[t]:03d} {tdec[t]:12.4f} {mass_anomaly:14.6f}', file=fid1
            )
            print(
                f'{mon[t]:03d} {tdec[t]:12.4f} {thick_anomaly:14.6f}', file=fid2
            )
            print(
                f'{mon[t]:03d} {tdec[t]:12.4f} {mbar_anomaly:14.6f}', file=fid3
            )
...
def arguments():
    parser = argparse.ArgumentParser(
        description="""Calculates the time-series of mass change
...
def main():
    ...
    try:
        info(args)
        # run combine_HEX_mascon_timeseries algorithm with parameters
        combine_HEX_mascon_timeseries(
            args.lmax,
            args.radius,
            MMAX=args.mmax,
