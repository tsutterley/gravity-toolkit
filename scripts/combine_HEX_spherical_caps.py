...
def combine_HEX_spherical_caps(
    PROC,
    DREL,
    DSET,
    LMAX,
    ...
    for k in cap['ALL']:
        ...
        # leakage file
        f2 = 'SPH_CAP_RAD{0:0.1f}_{1:d}_{2}{3}{4}L{5:d}{6}{7}{8}_LEAKAGE.txt'.format(
            *a
        )
    ...
    for i in region:
        ...
        for t in range(n_mon):
            ...
            print(
                f'{mon[t]:03d} {tdec[t]:12.4f} {mass_anomaly:14.6f} '
                f'{total_error[t]:14.6f} {area_km:16.5f}',
                file=fid1,
            )
            print(
                f'{mon[t]:03d} {tdec[t]:12.4f} {thick_anomaly:14.6f} '
                f'{thick_error[t]:14.6f} {area_km:16.5f}',
                file=fid2,
            )
