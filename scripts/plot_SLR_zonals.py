...
def plot_SLR_zonals(base_dir, PROC, DREL, START_MON, END_MON, MISSING):
    ...
    YY, MM, DD, hh, mm, ss = gravtk.time.convert_julian(
        C55['MJD'] + 2400000.5, format='tuple'
    )
    ...
    ax1 = {}
    ax2 = {}
    f1, (ax1[2], ax1[3], ax1[4], ax1[5]) = plt.subplots(
        num=1, nrows=4, sharex=True, figsize=(6, 8)
    )
    f2, (ax2[2], ax2[3], ax2[4], ax2[5]) = plt.subplots(
        num=2, nrows=4, sharex=True, figsize=(6, 8)
    )
    plot_colors = ['mediumseagreen', '0.5', 'darkorchid', 'darkorange']
    plot_labels = [
        f'{PROC} GRACE/GRACE-FO',
        f'{PROC} GRACE Climatology',
        'GSFC SLR',
        'GFZ GravIS',
    ]  #'CSR TN-11 SLR'
    plot_zorder = [0, 3, 1, 2]
    ...
    for l, ax in ax1.items():
        ...
        CLIMATE = {}
        YY = np.arange(2002 + 1.0 / 24.0, 2024, 1.0 / 12.0).astype(np.int64)
        ...
        CLIMATE['data'] = regress_model(
            GRACE['date'][ii],
            GRACE['data'][ii],
            CLIMATE['date'],
            ORDER=2,
            CYCLES=[0.25, 0.5, 1.0, 2.0, 4.0, 5.0, 161.0 / 365.25],
        ...
        if l == 2:
            ...
            GSFC = gravtk.convert_weekly(
                G55['time'], G55['data'], DATE=GRACE['date'], NEIGHBORS=28
            )
    ...
    f1.subplots_adjust(
        left=0.08, right=0.97, bottom=0.05, top=0.99, hspace=0.06
    )
    ...
    # calculate GRACE climatology for "good months"
    CLIMATE = {}
    YY = np.arange(2002 + 1.0 / 24.0, 2022, 1.0 / 12.0).astype(np.int64)
    CLIMATE['month'] = 1 + np.arange(len(YY))
    MM = 1 + ((CLIMATE['month'] - 1) % 12)
    CLIMATE['date'] = gravtk.time.convert_calendar_decimal(YY, MM)
    (ii,) = np.nonzero((GRACE['month'] >= 13) & (GRACE['month'] <= 176))
