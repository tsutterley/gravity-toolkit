#!/usr/bin/env python
"""
combine_HEX_Caron_errors.py (04/2024)
calculates the estimated GIA uncertainty for spherical cap mascon regions
...
cap['EEp'] = np.array(
    [106, 117, 118, 119, 120, 131, 132, 133, 134, 145, 146, 147]
)
...
cap['JppK'] = np.array(
    [49, 50, 51, 52, 62, 63, 64, 65, 66, 76, 77, 78, 79, 90, 91, 92, 104, 105]
)
...
cap['WAIS'] = np.concatenate(
    (cap['EpFp'], cap['FpG'], cap['GH'], cap['HHp'], cap['JJpp']), axis=0
)
cap['APIS'] = np.concatenate((cap['HpI'], cap['IIpp'], cap['IppJ']), axis=0)
cap['INTERIOR'] = np.array(
    [
        38,
        51,
        52,
...
cap['GIS'] = np.concatenate(
    (cap['NW'], cap['NN'], cap['NE'], cap['SW'], cap['SE']), axis=0
)
...
cap['DEN'] = np.array(
    [345, 346, 348, 349, 350, 351, 352, 353, 354, 355, 356, 367]
)
...
def combine_HEX_Caron_errors(
    ...
    for k in cap['ALL']:
        # GIA files
        args = (RAD_CAP, k, 'CARON_ERROR_', ocean_str, LMAX, order_str, gw_str)
        f1 = 'SPH_CAP_RAD{0:0.1f}_{1:d}_{2}{3}L{4:d}{5}{6}.txt'.format(*args)
        # read cap for GIA covariances
        GIA_data[k] = np.loadtxt(OUTPUT_DIRECTORY.joinpath(f1))
    ...
    for i in region:
        ...
        for k in cap[i]:
            # sum of GIA uncertainty data
            GIA_reg[i] += GIA_data[k][0] ** 2
            # add mascon area to total area (cm^2)
            area_reg[i] += 1e10 * GIA_data[k][1]

        # output data files
        args = ('Caron_Error_', i, ocean_str, LMAX, order_str, gw_str)
        FILE1 = '{0}{1}_SPH_CAP_{2}L{3:d}{4}{5}.txt'.format(*args)
        FILE2 = '{0}{1}_SPH_CAP_{2}L{3:d}{4}{5}_cmwe.txt'.format(*args)
        # open files for writing region error
        ...
        # output GIA regional error
        total_error = np.sqrt(GIA_reg[i])
        thick_error = 1e15 * total_error / area_reg[i]
        print(f'{total_error:14.6f}', file=fid1)
        print(f'{thick_error:14.6f}', file=fid2)
...
def arguments():
    parser = argparse.ArgumentParser(
        description="""Calculates the estimated GIA uncertainty for
...
def main():
    ...
    try:
        info(args)
        # run combine_HEX_Caron_errors algorithm with parameters
        combine_HEX_Caron_errors(
            args.lmax,
            args.radius,
            MMAX=args.mmax,
